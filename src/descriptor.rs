...
pub struct DescriptorWriter<'a> {
    ...
    prev_interface: Option<InterfaceNumber>,
}
...
impl DescriptorWriter<'_> {
    ...
    pub fn interface(
        ...
        if self.num_interfaces_mark == None {
            return Err(UsbError::InvalidState);
        }

        match self.prev_interface {
            Some(prev) if prev == number => (),
            Some(_) | None => {
                self.buf[self.num_interfaces_mark.unwrap()] += 1;
                self.prev_interface = Some(number);
            }
        }
        ...
        self.write(
            ...
            &[
                ...
                alternate_settings,  // bAlternateSetting
                0,                   // bNumEndpoints
